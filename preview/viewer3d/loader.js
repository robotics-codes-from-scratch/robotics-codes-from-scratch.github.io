...
import { enableToonShading, enableLightToonShading } from './helpers/materials.js';


function loadRobotModel(robot, parameters) {
            ...
            (xml) => {
                ...
                urdfLoader.loadMeshCb = (path, manager, done) => {
                    urdfLoader.defaultMeshLoader(path, manager, (mesh) => {
                        if (parameters.get('robot_use_toon_shader'))
                            enableToonShading(mesh);
                        else if (parameters.get('robot_use_light_toon_shader'))
                            enableLightToonShading(mesh);
...
export function loadRobot(parameters) {
    let robot = new Franka();

    return loadRobotModel(robot, parameters)
        .then(model => {
            robot.init(model);
