...
export default class Robot {
    ...
    */
    ...
    createJointPositionHelpers(scene, colors=[]) {
        for (let i = 0; i < this.arm.movable.length; ++i) {
            const joint = this.arm.movable[i];

            const helper = new JointPositionHelper(
                scene, joint, i + 1, this.jointPositionHelperInverted.includes(joint.name),
                colors[i] || 0xff0000
            );

            if (this.jointPositionHelperOffsets[joint.name])
                helper.setOffset(this.jointPositionHelperOffsets[joint.name]);
        }
    }
