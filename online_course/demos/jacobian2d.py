...
canvas = document.getElementById('canvas_jacobian2d')
...
def clear_screen():
    ...
    ctx.setTransform(canvas.width / 650, 0, 0, -canvas.width / 650, canvas.width*0.2, canvas.height*0.9)
...
#       ctx.restore()
    ...
    for i in range(param.nbVarX):
        ...
        xtmp = np.zeros(2)
        xtmp[0] = (f[0,i] + np.cos(np.mean(a))*(r+20)-15) * (canvas.width / 650) + canvas.width * 0.2
        xtmp[1] = (f[1,i] + np.sin(np.mean(a))*(r+20)-15) * (canvas.width / 650) + canvas.height * 0.1
...
#   ctx.restore()
    ...
    xtmp = np.zeros(2)
    xtmp[0] = (fhist[0,fhist_id-1] - 15) * (canvas.width / 650) + canvas.width * 0.2
    xtmp[1] = -60 * (canvas.width / 650) + canvas.height * 0.1
    f_el[0].setAttribute('style', 'position:absolute; left:%dpx; bottom:%dpx;' % (xtmp[0],xtmp[1])) # Horizontal axis
    xtmp[0] = -60 * (canvas.width / 650) + canvas.width * 0.2
    xtmp[1] = (fhist[1,fhist_id-1] - 15) * (canvas.width / 650) + canvas.height * 0.1
